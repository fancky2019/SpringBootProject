...
//三级缓存： 为循环依赖场景提供提前创建代理对象的机会
//       正常在 后置处理器的 postProcessAfterInitialization创建
//      循环依赖 代理创建跑到了 initializeBean() 前面

//        三级缓存  解决 AOP 代理对象在循环依赖场景下无法提前暴露，导致依赖方注入原始对象、从而引起事务、异步、缓存等切面失效的问题。
        /*
         三级缓存作用：三级缓存存在的根本原因，是为了在循环依赖场景下支持 AOP 代理的提前曝光。解决代理切面在循环依赖场景下的失效问题
        目的：解决循环依赖时，依赖方拿到原始对象而非代理对象，导致 AOP 切面（事务、异步等）完全失效的问题。
        原理：将代理生成的时机从「初始化后」提前到「实例化后、依赖注入前」，通过三级缓存的 getEarlyBeanReference 钩子实现。
            将原本应在 “初始化后(postProcessAfterInitialization)” 才创建的代理，
            提前到 “依赖注入阶段(populateBean期间)” 创建。
       本质：就是为 AOP 切面信息服务的，确保代理在循环依赖中尽早曝光。
         */
...
总结二级缓存只是个仓库，它不创建代理。代理在初始化阶段创建，这个时机对解决带AOP的循环依赖来说太晚了。	三级缓存通过工厂模式，将代理的创建时机从“固定的初始化后”提前到“被需要时”，从而完美解决了问题。
二级缓存本身不创建代理，它的存储能力也无法改变代理创建的时机问题。 这正是Spring引入三级缓存，通过工厂模式来控制代理生成时机的根本原因。
