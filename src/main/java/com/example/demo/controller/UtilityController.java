...
//mybatis plus
     ...
     * NOT_SUPPORTED： 有了挂起，没有就无事务运行  ,连接池开启Connection-B 会处于默认的自动提交（auto-commit）模式
     * NEVER： 以非事务方式执行，如果存在事务则抛出异常3
     * Propagation.NESTED: 调用者事务存在调用者事务和被调用者事务分别在两个事务中执行，嵌套事务回滚到回滚点。外层事务回滚整个事务。
     *
     *
     *PROPAGATION_NESTED 是 Spring 事务中一个非常特殊且容易被误解的传播行为。它与 REQUIRES_NEW 有本质区别，
     * 不是通过开启新连接实现的，而是利用底层 JDBC 驱动的保存点（Savepoint） 机制，在同一个物理连接的同一个事务中实现的。
     *
     * 检查当前是否存在事务：
     * 如果没有事务：行为完全等同于 REQUIRED。它会直接开启一个全新的事务（获取新连接，开始事务）。
     * 如果已存在事务：这是 NESTED 发挥作用的场景。它不会挂起当前事务，也不会获取新连接，而是复用当前线程上已绑定的数据库连接。
     *
     * 在当前事务中创建保存点（Savepoint）：
     * 事务管理器会通过当前连接的 setSavepoint() 方法，在当前数据库事务中创建一个命名保存点。这是 NESTED 最核心的操作。
     * 这个保存点标记了当前事务中的一个“回滚位置”。之后，嵌套事务中的所有数据库操作，都发生在同一个物理事务中，但被这个保存点“标记”了出来。
     *
     * 执行嵌套事务内的业务代码：
     * 被 @Transactional(propagation = Propagation.NESTED) 标记的方法内的所有数据库操作，都使用同一个连接，在同一个全局事务中执行。
     *
     * 提交/回滚嵌套事务：
     * 如果嵌套事务执行成功：Spring 会释放这个保存点（清理相关资源），但不会提交整个事务。最终的提交或回滚，仍然由最外层的事务决定。
     * 如果嵌套事务执行失败，需要回滚：事务管理器会调用 connection.rollback(savepoint) 方法，仅将数据库状态回滚到该保存点。外层事务的其他部分（包括之前执行的操作）不受影响，可以继续执行
     *
     */
