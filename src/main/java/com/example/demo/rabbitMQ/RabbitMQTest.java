...
消息删除      ┘
 ...
 消息积压：
 一、不可靠 1、设置Listener 迁移标志位，通过接口请求
     2、写入老队列，等老队列迁移完，就写入信队列
     3、消费新队列
 二、1、将消息批量获取，批量写入多个本地消息表（根据id取模写入多个消息表）
    2、开启定时任务消费
方案二比方案一综合优
...
判断组内是否有重复业务ID
↓
├─ 有重复ID → 同步顺序发送（保证顺序，失败即停）
└─ 无重复ID → 异步并发发送（提升吞吐，各自独立重试）
核心原则（必须遵守）:前一条没成功 → 后一条绝不能发送


RabbitMQ 默认的轮询（Round-robin）负载均衡机制:
队列: [m1, m2, m3, m4, m5, m6]
消费者订阅队列：多个消费者（c1、c2、c3）订阅同一个队列

消息依次分发：RabbitMQ 按照消息到达顺序，轮询将消息分配给各个消费者
...
m6 → c3
...以此类推
推送模式：RabbitMQ 会主动将消息推送给消费者，而不是等消费者拉取

配合 QoS（预取计数）解决不平衡:prefetch_count=1
消费者一次只接收 1 条消息

处理完并确认（ack）后，才接收下一条

处理快的消费者会自然获得更多消息，实现能者多劳
